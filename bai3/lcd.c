...
// Transmit a number of rows to the memory LCD display.
// @param[in] device
// Memory LCD display device.
// @param[in] data
// Pointer to the pixel matrix buffer to draw. The format of the
// buffer
...
sl_status_t lcd_tx(const struct sl_memlcd_t* device, const void* data, unsigned int row_start, unsigned int row_count) {
    ...
    if (row_count == 1) {
        ...
        uint8_t frame[20] = {0};
        frame[0] = cmd;
        frame[1] = (uint8_t)row_start;  // Address
        memcpy(&frame[2], pixel_data, 16);
        /* Send command */
        sli_memlcd_spi_tx(&spi_handle, (void*)frame, sizeof(frame));
        sli_memlcd_spi_wait(&spi_handle);
    } else {
        // Data update mode (multiple lines)
        uint8_t frame[row_count * (2 + 16) + 2] = {0};
        uint8_t* p = frame;

        *p++ = CMD_UPDATE;  // M0=1
        for (unsigned int r = 0; r < row_count; r++) {
            const uint8_t *line_data = pixel_data + ( * bytes_per_row); // i = 0 -> rowcount - 1
            *p++ = (uint8_t)(row_start + r);  // Address
            memcpy(p, (uint8_t*)line_data + r * 16, 16);
p += 16;
        }
        /* Send command */
        sli_memlcd_spi_tx(&spi_handle, (void*)frame, sizeof(frame));
...
static void on_power_manager_event(sl_power_manager_em_t from,
    ...
    if (to == SL_POWER_MANAGER_EM1 || to == SL_POWER_MANAGER_EM0) {
